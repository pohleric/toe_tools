...
for season in ['annual', 'summer', 'winter']:
    for confidence_level in confidence_levels:
        ...
        ref_pd = pd.read_pickle(str(CRU_HD_Tair) + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s_.pickle" % (
            CRU_var_name_T, season, XSPLIT_year, WW))
        ref_pd_sign = pd.read_pickle(
            str(CRU_HD_Tair) + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s__sign.pickle" % (
                CRU_var_name_T, season, XSPLIT_year, WW))
        ...
        files = [f for f in listdir(CRU_HD_Tair) if
                 re.match(r'^%s_CRU-NCEP_.+_Siberia_hellinger_%s.+split.+_.pickle$' % (CRU_var_name_T, season), f)]
        files.sort()
        ...
        for model_i in files:
            print(model_i)
            name_pickle = CRU_HD_Tair + model_i
