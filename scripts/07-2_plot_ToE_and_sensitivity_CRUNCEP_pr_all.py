...
for season in ['annual', 'summer', 'winter']:
    ...
    for confidence_level in confidence_levels:
        ...
        # ALL models
        # MAX value
        _dname = '%s_val_%s_%s_%s' % (CRU_var_name_P, model, confidence_level, season)
        ######################
        ...
        # TOE
        _dname = '%s_ToE_%s_%s_%s' % (CRU_var_name_P, model, confidence_level, season)
        ######################
    ...
    for year_i in years:
        # year_i= 2000
        _dname = '%s_val_avg-HD_mean_%s_%s_' % (CRU_var_name_P, model, season)
        ######################
        if year_i <= 2004:
