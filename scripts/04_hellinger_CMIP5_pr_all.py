...
# #### PARAMETERS
_aggregate = agg_fun(ESD_var_name_P)
...
for season in ['annual', 'summer', 'winter']:
    ...
    files = [f for f in listdir(ESD_TXT) if
             re.match(r'^%s_ESD_[0-9]{3}_1861-2100_Siberia_df_monthly' % ESD_var_name_P, f)]
    files.sort()
    OUT_overlap_pd_list = []
    ...
    for f0 in tqdm(files):
        ...
        name_pickle = ESD_HD_pr + f_base + ".pickle"
        name_pickle_sign = ESD_HD_pr + f_base + "_sign.pickle"
        OUT_overlap_pd.to_pickle(name_pickle, protocol=2)
        OUT_overlap_sign.to_pickle(name_pickle_sign, protocol=2)
        name_csv = ESD_HD_pr + f_base + ".csv"
        name_csv_sign = ESD_HD_pr + f_base + "_sign.csv"
    ...
    file_ens = ESD_HD_pr + "%s_ESD_mean_1861-2100_Siberia_df_monthly-orig.csv" % ESD_var_name_P
    ens_pd = pd_hydroYear(file_name=file_ens, season=season, aggregate=_aggregate)
    ...
    name_pickle = ESD_HD_pr + file_ens_base + ".pickle"
    name_pickle_sign = ESD_HD_pr + file_ens_base + "_sign.pickle"
    ens_overlap.to_pickle(name_pickle, protocol=2)
    ens_overlap_sign.to_pickle(name_pickle_sign, protocol=2)
    name_csv = ESD_HD_pr + file_ens_base + ".csv"
    name_csv_sign = ESD_HD_pr + file_ens_base + "_sign.csv"
