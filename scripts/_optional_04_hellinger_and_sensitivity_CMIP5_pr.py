...
files = [f for f in listdir(PATH_ESD) if re.match(r'^%s_ESD_[0-9]{3}_1861-2100_Siberia_df_monthly' % ESD_var_name_P, f)]
files.sort()
...
for XSPLIT_year in tqdm(np.arange(1915, 1929.1, 2).astype('int')):
    ...
    for WW in np.arange(17, 29.1, 2).astype('int'):
        ...
        file_ens = PATH_ESD + "%s_ESD_mean_1861-2100_Siberia_df_monthly-orig.csv" % ESD_var_name_P
