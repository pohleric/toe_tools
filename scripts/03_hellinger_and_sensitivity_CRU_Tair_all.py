...
# #### PARAMETERS
_aggregate = agg_fun(CRU_var_name_T)

# END PARAMETERS _________________________________
for season in ['annual', 'summer', 'winter']:
    pd_df = pd_hydroYear(CRU_TXT + "%s_CRU-NCEP_1901-2016_Siberia_df_monthly-orig.csv" % CRU_var_name_T, season=season,
    ...
    for XSPLIT_year in tqdm(np.arange(1915, 1931, 2)):
        ...
        for WW in np.arange(15, 31, 2):
            ...
            name_pickle = CRU_HD_Tair + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s_.pickle" \
                          % (CRU_var_name_T, season, XSPLIT_year, WW)
            name_pickle_sign = CRU_HD_Tair + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s__sign.pickle" \
                               % (CRU_var_name_T, season, XSPLIT_year, WW)
            OUT_overlap_pd.to_pickle(name_pickle, protocol=2)
            OUT_overlap_pd_sign.to_pickle(name_pickle_sign, protocol=2)
            name_csv = CRU_HD_Tair + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s_.csv" \
                       % (CRU_var_name_T, season, XSPLIT_year, WW)
            name_csv_sign = CRU_HD_Tair + "%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s__sign.csv" \
                            % (CRU_var_name_T, season, XSPLIT_year, WW)
    ...
    file_ens_base = "%s_CRU-NCEP_average_1901-2016_Siberia_df_%s-orig" % (CRU_var_name_T, season)

    name_pickle = CRU_HD_Tair + file_ens_base + ".pickle"
    name_pickle_sign = CRU_HD_Tair + file_ens_base + "_sign.pickle"
    df_mean_mon.to_pickle(name_pickle, protocol=2)
    df_sign_mean_mon.to_pickle(name_pickle_sign, protocol=2)
    name_csv = CRU_HD_Tair + file_ens_base + ".csv"
    name_csv_sign = CRU_HD_Tair + file_ens_base + "_sign.csv"
