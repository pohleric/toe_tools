...
for season in ['annual', 'summer', 'winter']:
    ...
    cru = read_csv_ann(CRU_HD_Tair + '%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s_.csv' % (
        CRU_var_name_T, season, XSPLIT_year, WW))
    cru_sign = read_csv_ann(CRU_HD_Tair + '%s_CRU-NCEP_1901-2016_Siberia_hellinger_%s_split%s_WW%s__sign.csv' % (
        CRU_var_name_T, season, XSPLIT_year, WW))
    ...
    # PATH_ESD = "/home/hydrogeol/epohl/data/ESD/sensitivity_Lena_hellinger/fullSeries/"
    # files = [f for f in listdir(PATH_ESD) if re.match(r'^%s_ESD_[0-9]{3}_1861-2100_Siberia_df_%s.+orig.csv$' % (var_name_ESD, season), f)]
    files = [f for f in listdir(ESD_HD_tas) if
             re.match(r'^%s_ESD_[0-9]{3}_1861-2100_Siberia_df_%s.+orig.csv$' % (ESD_var_name_T, season), f)]
    files.sort()

    fname = ESD_HD_tas + files[0]
    ...
    for loc_i, station in zip(locations, stations):
        ...
        #
        # ax1.set_title(r'%s - %s (NSE) @ %s' % (ESD_var_name_T, season, station[0]), fontsize=cex_main_text)
        ...
        # ESD_var_name_T, n_models, season, station[0]))
        # plt.close()
        #
        #
        ...
        # with open('data/%s_best_%s_models_%s_NSE_%s_LongStations.txt' % (ESD_var_name_T, n_models, season, station[0]),
        #           'w') as outtxt:
        #     for item in ord_nse:
        #         # print(item)
        #         # outtxt.write("%s\n" % item)
        ...
        #
        # # write best models into txt
        # with open('data/%s_best_%s_models_%s_R2_%s_LongStations.txt' % (ESD_var_name_T, n_models, season, station[0]),
        #           'w') as outtxt:
        #     for item in ord_r2:
        #         # print(item)
        #         # outtxt.write("%s\n" % item)
    ...
    models_nse_stats_pd.to_csv(
        'data/%s_best_%s_models_%s_NSE_stats_LongStations.txt' % (ESD_var_name_T, n_models, season))
    ...
    pd.DataFrame(str_str).to_csv('data/%s_best_%s_models_%s_NSE_LongStations.txt' % (ESD_var_name_T, n_models, season),
                                 header=False, index=False)
    df_sort.to_csv('data/%s_all_models_%s_NSE_LongStations.txt' % (ESD_var_name_T, season), header=False, index=True)
    ...
    # write best models into txt
    str_str = df_sort.iloc[0:n_models, 0].index.values
    pd.DataFrame(str_str).to_csv('data/%s_best_%s_models_%s_R2_LongStations.txt' % (ESD_var_name_T, n_models, season),
                                 header=False, index=False)
    df_sort.to_csv('data/%s_all_models_%s_R2_LongStations.txt' % (ESD_var_name_T, season), header=False, index=True)
...
pd.DataFrame(all_table_NSE).to_csv(
    'data/%s_best_%s_models_all_seasons_NSE__LongStations.txt' % (ESD_var_name_T, n_models), header=False, index=False)
all_table_R2 = stats_list_R2.sum(0).sort_values(0, ascending=False)[0:n_models].index
pd.DataFrame(all_table_R2).to_csv(
    'data/%s_best_%s_models_all_seasons_R2__LongStations.txt' % (ESD_var_name_T, n_models), header=False, index=False)
