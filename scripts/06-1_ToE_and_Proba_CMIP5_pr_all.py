...
for season in ['annual', 'summer', 'winter']:
    for confidence_level in confidence_levels:
        ...
        ref_pd = pd.read_pickle(
            str(ESD_HD_pr) + "%s_ESD_average_1861-2100_Siberia_df_%s-orig.pickle" % (ESD_var_name_P, season))
        datetime_reference = ref_pd.index
        ref_pd_sign = pd.read_pickle(
            str(ESD_HD_pr) + "%s_ESD_average_1861-2100_Siberia_df_%s-orig_sign.pickle" % (ESD_var_name_P, season))
        ...
        # ref_pd = pd.read_pickle(str(ESD_HD_pr)+"%s_ESD_%s_1861-2100_Siberia_df_%s-orig.pickle" % (ESD_var_name_P, best_model_0,season))
        # datetime_reference = ref_pd.index
        # ref_pd_sign = pd.read_pickle(str(ESD_HD_pr)+"%s_ESD_%s_1861-2100_Siberia_df_%s-orig_sign.pickle" % (ESD_var_name_P, best_model, season))
        ...
        #     # plt.scatter(x, y)
        # name_csv = ESD_ToE+"%s_ESD_ToE_Sensitivity-valatyears_%s_1901-2016_Siberia_df_%s_overlap_.csv" % (ESD_var_name_P, best_model, season)
        # name_csv_sign = ESD_ToE+"%s_ESD_ToE_Sensitivity-signatyears_%s_1901-2016_Siberia_df_%s_overlap_.csv" % (ESD_var_name_P, best_model, season)
        # val_at_years.to_csv(name_csv)
        # sign_at_years.to_csv(name_csv_sign)
        ...
        for best_model in best_models:
            ref_pd = pd.read_pickle(
                str(ESD_HD_pr) + "%s_ESD_%s_1861-2100_Siberia_df_%s-orig.pickle" % (
                    ESD_var_name_P, best_model, season))
            datetime_reference = ref_pd.index
            ref_pd_sign = pd.read_pickle(
                str(ESD_HD_pr) + "%s_ESD_%s_1861-2100_Siberia_df_%s-orig_sign.pickle" % (
                    ESD_var_name_P, best_model, season))
            val_at_years = pd.DataFrame(columns=years)
            sign_at_years = pd.DataFrame(columns=years)
        ...
        files = [f for f in listdir(ESD_HD_pr) if
                 re.match(r'^%s_ESD_[0-9]{3}_1861-2100_Siberia_df_%s.+orig.pickle$' % (ESD_var_name_P, season), f)]
        files.sort()
